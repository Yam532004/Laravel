...
<form action="{{route('users.post-edit')}}" method="POST">
    ...
    <div class="mb-3">
        ...
        <select name="status" id="" class="form-control">
            <option value="0" {{ old('status') == 0 || $userDetail->status == 0?'selected':false }} >Chua kich hoach</option>
            <option value="1" {{ old('status') == 1 || $userDetail->status == 1?'selected':false }}>Kich hoach</option>
        </select>
        @error('group_id')
            <span style="color: red;">{{$message}}</span>
        @enderror
